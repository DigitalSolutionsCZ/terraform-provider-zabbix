...
install-plugin:
	mkdir -p ~/.terraform.d/plugins/localdomain/local/zabbix/0.1.0/linux_amd64/
	cp terraform-provider-zabbix ~/.terraform.d/plugins/localdomain/local/zabbix/0.1.0/linux_amd64/
...
go-fmt-check:
	...
	@unformatted=$$(find . -type f -name '*.go' -not -path './zabbix_data/*' 2>/dev/null | xargs gofmt -s -l); \
...
go-fmt:
	...
	@find . -type f -name '*.go' -not -path './zabbix_data/*' 2>/dev/null | xargs gofmt -s -w
	@echo "Done."
